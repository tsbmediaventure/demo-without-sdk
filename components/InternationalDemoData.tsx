import International1 from "../public/assets/blog/International1.webp";
import International2 from "../public/assets/blog/International2.webp";
import International3 from "../public/assets/blog/International3.webp";
import International4 from "../public/assets/blog/International4.webp";
import International5 from "../public/assets/blog/International5.webp";

export const InternationalData = [
  {
    id: 'v1',
    title:
      "Biden's State of Union address: What he said on Chinese aggression, US economy and police brutality ",
    subTitle: `President Joe Biden delivered his second State of the Union address. In his 73-minute speech, he spoke on a range of issues, including US-China relations, the US economy, the`,
    name: "Anuradha shetty",
    date: "feb 5,2023",
    img: International1,
  },
  {
    id: 'v2',
    title: "Rishi Sunak shuffles UK's cabinet, announces 4 new departments",
    subTitle:
      "Britain's Prime Minister Rishi Sunak has shuffled his cabinet. With his prime focus on the economy, Sunak has created a new department for Energy Security and Net Zero.",
    name: "Divesh Naidu ",
    date: "feb 05, 2023",
    img: International2,
  },
  {
    id: 'v3',
    title:
      "3.2 tonnes of cocaine, enough to supply market for 30 years, found floating at sea in New Zealand",
    subTitle:
      "Authorities in New Zealand have recovered 3.2 tonnes of cocaine floating at the sea. The drugs were collected from the Pacific Ocean. No arrests have so far been made in the case.",
    name: "Samaksh Gupta",
    date: "feb 03, 2023",
    img: International3,
  },
  {
    id: 'v4',
    title:
      "Hundreds attend Pakistan’s former President Pervez Musharraf's funeral",
    subTitle:
      "Pakistan’s former President Pervez Musharraf was buried on Tuesday in his family’s hometown. Several politicians and government officials paid glowing tributes to him.",
    name: "Sambuddha Chakraborty",
    date: "feb 09, 2023",
    img: International4,
  },
  {
    id: 'v5',
    title:
      "At least 15 killed in Peru landslide, president pledges to speed up help",
    subTitle:
      "The death toll from a massive landslide that struck southern Peru rose to at least 15 on Tuesday after dozens of homes were swallowed up by dirt and mud following heavy rainfall last weekend.",
    name: "Sunny Sen",
    date: "feb 05, 2023",
    img: International5,
  },
];
